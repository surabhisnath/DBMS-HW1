   ...
   while(!f3.eof())
    {
        ...
        int index=tempcnt%no_fields;

        const char *compi = arr2[tempcnt].c_str();
        if(strcmp(compi,"int")==0)
        {
            if(atoi(p.c_str())> pow(2,8*(atoi(arr3[tempcnt].c_str())-1)))
            {
                cout<<"ERROR: Not enough bytes assigned for the int value"<<endl;
                return 0;
            }
        }

        if(strcmp(compi,"string")==0)
        {
            if((p.length())> atoi(arr3[tempcnt].c_str()))
            {
                cout<<"ERROR: Not enough bytes assigned for the string value"<<endl;
                return 0;
            }
        }
